...
define([
	...
	'./toggle-completion',
	'./chatboxSpeed'
], function(Adapt, AdaptModel, DevtoolsModel, PassHalfFail, ToggleBanking, Map, ChatboxSpeed) {
	...
	var DevtoolsView = Backbone.View.extend({
		...
		events:{
			...
			'change .chatboxSpeed input':'chatboxSpeedToggle',
		...
		_checkChatboxVisibility: function() {
			if (Adapt.componentStore.chatbox) {
				this.$('.toggle.chatboxSpeed').removeClass('display-none');
				this.$('.toggle.chatboxSpeed label').toggleClass('selected', false);
				this.$('.tip.chatboxSpeed').toggleClass('display-none');
			}
			else {
				this.$('.chatboxSpeed').addClass('display-none');
			}
		},

		chatboxSpeedToggle: function() {
			this.$('.toggle.chatboxSpeed label').toggleClass();
			Adapt.trigger('devtools:chatboxSpeed');
		},
